...
Por ejemplo podríamos pensar que una `NaveConEscudo` se dibuja como una `Nave` con algunos elementos extras. En este caso sería más interesante el reﬁnamiento que el reemplazo, ya que aprovecharíaamos el código del método `dibuja()` escrito para la clase `Nave`.
