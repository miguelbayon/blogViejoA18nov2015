...
<table id="uml">	
	...
	<tr>
		<td>
		+PhotoPost(String author, String filename, String caption)<br>
		+like()<br>
		+unlike()<br>
		+addComment(String text)<br>
...
En nuestro caso, en vez de definir dos clases completamente independientes `MessagePost` y `PhotoPost` lo primero que hacemos es definir una clase que contiene todo aquello que estas dos clases tienen en común. Llamaremos a esta clase `Post`. 

Luego le diremos a Java que cualquier objeto `MessagePost` es un `Post` y que cualquier objeto `PhotoPost` también es un `Post`. Finalmente añadiremos aquellos detalles adicionales propios de `MessagePost` en esa clase y aquellos detalles adicionales propios de `PhotoPost` en esa otra clase.

La herencia nos permite por tanto escribir todo aquello que ambas clases tienen en común solo una vez.

A continuación se muestra el diagrama UML de la nueva estructura. Encima de todo se muestra la clase `Post`, que define todos los atributos y métodos comunes a todos los tipos de post. Debajo se muestran las clases `MesssagePost` y `PhotoPost` conteniendo aquellos atributos y métodos propios únicamente de dichas clases.

<img src="/images/Captura01.png" class="img-responsive">

La herencia, presente en todos los lenguajes de programación orientados a objetos, define una serie de conceptos. En nuestro ejemplo, decimos que la clase `MessagePost` _hereda_ de la clase `Post`; también decimos que la clase `PhotoPost` _hereda_ de la clase de `Post`. Las flechas representadas en el diagrama representan las relaciones de herencia.

La clase `Post` es denominada la _clase padre_ o la _superclase_ mientras que las clases que heredan, `MessagePost` y `PhotoPost` se denominan _clases hijas_ o _subclases_.

Es importante darse cuenta que las subclases son una especialización de la superclase. Podemos decir que un objeto de tipo `MessagePost` _es un_ objeto de tipo `Post` y que un `PhotoPost` _es un_ `Post` igualmente.

Uno de los propósitos de la herencia se muestra claramente en el ejemplo. Los objetos de tipo `MessagePost` tienen todos los atributos definidos en su clase y, además, los atributos definidos en su superclase, es decir, los definidos en la clase `Post`. Así, los atributos `username`, `timestamp`, `likes` y `comments` de definen y se inicializan solo una vez pero podemos usarlos en dos clases: `MessagePost` y `PhotoPost`.
                                                                                                                                                                                                                                                                                                                                                                                                                                                                    c 
...
2. Fija un punto de ruptura (breakpoint) en la primera línea del constructor de `MessagePost`. Crea un objeto `MessagePost`. Utiliza luego el botón "Step into" para comprobar paso a paso comos e inicializa el objeto. Fíjate en los atributos y cómo van tomando valores. 
...
2. Queremos añadir dos nuevos tipos de post a la red social:
	...
	1. Posts que incluyen además de un texto contengan un enlace (una dirección URL).

	2. Posts que describen un evento de la red social. De momento solo vamos a conetmplar los siguientes eventos: alguien se hizo amigo de otro, alguien se unió a un determinado grupo o alguien cambio su foto de perfil.
